<script lang="ts">
    ...
    import DropdownArrow from "src/components/base/controls/DropdownArrow.svelte";
    ...
    <div class="options" transition:slide={{ duration: 200 }}>
        {#each $instances as instance}
            <Button
                outline
                compact
                extraStyle="box-shadow: 0 2px 6px rgba(0, 0, 0, .2);"
                onClick={() => {
                    open = false;
        ...
        {/each}
        <Button green nobg onClick={() => (isAddModalOpen = true)}>
